...
def cross_entropy(pred,
    ...
    if (avg_factor is None) and reduction == 'mean':
        if class_weight is None:
            ...
            # === 这里是修改后的代码 ===
            valid_mask = label != ignore_index # 创建一个布尔掩码（mask），标记哪些像素/样本是有效的
            valid_labels = label[valid_mask] # 利用上一步得到的布尔掩码，提取出所有有效位置的类别 ID # 这一步保证后续不会把 -100 这样的非法索引用到 class_weight 里
            label_weights = class_weight[valid_labels] # 根据有效标签，从类别权重数组 class_weight 中取出每个标签对应的权重

            if avg_non_ignore:
                # 仅对有效的类别权重求和 # 分母只统计非 ignore_index 的样本/权重
                avg_factor = label_weights.sum()
            else:
                # 创建一个和 label 形状一样的全 0 张量
                expanded_weights = torch.zeros_like(label, dtype=class_weight.dtype, device=class_weight.device)
                # 把 label_weights 的值按顺序填回 expanded_weights 中那些 valid_mask==True 的位置。被忽略的位置（False）仍然保持 0。
                expanded_weights[valid_mask] = label_weights
                # 对 expanded_weights 全部元素求和，得到一个标量（同 dtype/device）
                avg_factor = expanded_weights.sum()
