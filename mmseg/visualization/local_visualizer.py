        ...
        if with_labels:
            ...
            for mask_num in range(len(labels)):
                ...
                mask = cv2.putText(mask, text, (loc[0], loc[1] + label_height),
                                   ...
                                   lineType)
        # color_seg = (image * (1 - self.alpha) + mask * self.alpha).astype(np.uint8) # 标准的透明度叠加计算公式，但是我们的预测结果中有背景值（255像素）也参与了计算
        # self.set_image(color_seg)

        # 透明度叠加，只对非忽略区域叠加
        ignore_mask = (sem_seg[0] == 255)
        blended = image.copy().astype(np.float32)
        blended[~ignore_mask] = (image[~ignore_mask].astype(np.float32) * (1 - self.alpha) +
                                 mask[~ignore_mask].astype(np.float32) * self.alpha)
        color_seg = blended.astype(np.uint8)
        self.set_image(color_seg)
        return color_seg
