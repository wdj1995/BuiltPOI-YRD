...
from transformers import AutoConfig, AutoModel, AutoTokenizer, AutoProcessor, SamConfig, SamModel, SiglipModel, \
    SamProcessor, SiglipProcessor
...
# python tools/train.py configs/config-b.py
# bash tools/dist_train.sh configs/config-b.py 2
# python tools/test.py --config configs/config-b.py work_dirs/config-b/best_SegIoUMetric_Macro_F1_epoch_5.pth

# python demo/inference.py --config configs/config-b-inference.py --checkpoint z_backup_files/new-model-version-0926/01-no-building-mask/best_ClsMetric_Macro_F1_epoch_19.pth --out-dir inference_results --show-dir inference_results
# python demo/inference.py --config configs/config-b-inference.py --checkpoint work_dirs/config-b/best_ClsMetric_Macro_F1_epoch_6.pth --out-dir inference_results --show-dir inference_results
# python demo/inference.py --config configs/config-b-inference.py --checkpoint work_dirs/PHASE1_SEG_ONLY/epoch_9.pth --out-dir inference_results --show-dir inference_results
# IMAGE_FILE 为你想要预测的图像文件，name_to_config.py 为你想要使用的配置文件，CHECKPOINT_FILE 为你想要使用的检查点文件，OUTPUT_DIR 为预测结果的输出路径

# bash tools/dist_train.sh configs/config-b.py 4
# torch.backends.cudnn.enabled = False
# torch.backends.cudnn.benchmark = False
# torch.backends.cudnn.deterministic = True
# os.environ['CUDA_LAUNCH_BLOCKING'] = "1" # debug使用，训练时注释
...
class RefSegEncoderDecoder(EncoderDecoder):
    def __init__(
            ...
            seg_head: dict,
            cls_head: dict,
            dummy_head=dict(type='FCNHead', in_channels=256, channels=16, num_classes=2),
            # 假的分类头，防止继承EncoderDecoder出现decode_head报错
            *args,
    ...
    def set_finetune_parameters(self):
        ...
        for k in peft_keys:
            if hasattr(self, k):
                ...
                # 冻结BERT，但是解冻projection和logit_scale
                else:
                    # 1. 默认操作：先完全冻结该模块的所有参数
                    print(f"Info: {k} not in lora_cfg, freezing all parameters first.")
                    wrapper_module.requires_grad_(False)
                    # 2. 特殊处理：如果是文本编码器，必须解冻 Projection 层
                    if k == 'clip_text_encoder':
                        # 检查是否存在 projection 层 (RefSegBertTextModel 特有)
                        if hasattr(wrapper_module, 'projection') and wrapper_module.projection is not None:
                            print(f"--> Force unfreezing {k}.projection for feature alignment.")
                            # 遍历 projection 层的参数并设为 True
                            for param in wrapper_module.projection.parameters():
                                param.requires_grad = True
                        # 检查是否存在模拟的 logit_scale (建议解冻，利于收敛)
                        if hasattr(wrapper_module, 'logit_scale'):
                            print(f"--> Force unfreezing {k}.logit_scale/bias.")
                            wrapper_module.logit_scale.requires_grad = True
                            if hasattr(wrapper_module, 'logit_bias'):
                                wrapper_module.logit_bias.requires_grad = True
    ...
    def extract_feat(self, inputs, data_samples: SampleList, return_attns: bool = False) -> List[Tensor]:
        ...
        x_sam = normalize(inputs, mean=self.backbone.processor.image_mean,
                          std=self.backbone.processor.image_std)  # 按照 SAM 预处理规范对输入图像进行归一化
        # print("x_sam: ", x_sam)
        ...
        x_clip = normalize(inputs, mean=self.clip_vision_encoder.processor.image_mean,
                           std=self.clip_vision_encoder.processor.image_std)
        # 缩放图像：插值到 SigLIP 要求的固定输入尺寸
        x_clip = F.interpolate(x_clip, size=list(self.clip_vision_encoder.processor.size.values()), mode='bicubic',
        ...
        text_dict = self.clip_text_encoder.processor(text_list, return_tensors='pt', padding=True, truncation=True,
                                                     max_length=64)
        # print("text_dict.shape: ", text_dict)
        ...
        # print("clip_text_feat_pooler.shape:", clip_text_feat_pooler.shape)
        ####################################### POI 地理文本语义编码
        ...
        image_positional_embeddings = self.get_image_positional_embeddings(
            sam_visual_feat.size(2))  # 生成与 SAM 图像特征分辨率匹配的位置编码
    ...
    def predict(self, inputs: Tensor, data_samples: SampleList):
        ...
        if data_samples is not None:
            ...
            # 构造默认元数据
            batch_img_metas = [dict(ori_shape=inputs.shape[2:], img_shape=inputs.shape[2:], pad_shape=inputs.shape[2:],
        ...
        cls_scores, reasoning_vis = self.cls_head.forward(feat_tuple)
        # 保存 reasoning 可视化信息
        for k, v in reasoning_vis.items():
            vis_dict[k] = v.detach().cpu()
        ...
        for i, data_sample in enumerate(data_samples):
            ...
            data_sample.pred_vis = {
                k: v[i] if isinstance(v, torch.Tensor) and v.shape[0] == len(data_samples) else v
...
class ReasoningClsHead(BaseModule):
    ...
    def forward(self, inputs):
        ...
        local_s_up = F.interpolate(local_s, size=target_sz, mode='bilinear',
                                   align_corners=False)  # 统一缩放到 SAM 特征图尺寸 (64x64)
        ...
        updated_protos, _ = self.reasoning_engine(prototypes, final_evidence,
                                                  final_evidence)  # 核心推理：Query(原型) 在 Key/Value(视觉证据池) 中进行匹配
...
@MODELS.register_module()
class RefSegBertTextModel(BaseModule):
    def __init__(
            self,
            model_name_or_path: str = 'google-bert/bert-base-uncased',  # uncased 不区分大小写
            target_dim: int = 1024,  # 视觉编码器的维度 (例如 SigLIP 是 1024)
            init_cfg=None
    ):
        super().__init__(init_cfg=init_cfg)
        ...
        bert_dim = self.config.hidden_size  # 通常是 768
        # 3. 维度对齐 siglip为1024.bert为768
        self.target_dim = target_dim
        self.projection = nn.Linear(bert_dim, target_dim, bias=False)
        # 4. 模拟 CLIP 的温度参数 (Compatibility)
        # RefSegEncoderDecoder 的 extract_feat 中使用了这两个参数计算 global_gate
        # BERT 没有这些，我们需要初始化为可学习参数，模拟 CLIP 的默认行为
    ...
    def forward(self, input_ids, attention_mask=None, token_type_ids=None, **kwargs):
        # 过滤掉 BERT 不需要但 SigLIP 可能传进来的参数（如果有）
        # BERT 主要需要 input_ids 和 attention_mask
        outputs = self.model(
            input_ids=input_ids,
        ...
        # 重新封装成类似 CLIP/SigLIP 的输出格式 (字典或对象均可，只要能用 ['key'] 访问)
        return {
...
if __name__ == '__main__':
    siglip_vision_encoder = RefSegSiglipVisionModel(model_name_or_path='lcybuaa1111/Git-RSCLIP')
