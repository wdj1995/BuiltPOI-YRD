...
class RefSegEncoderDecoder(EncoderDecoder):
    def __init__(
        ...
        # 自动获取 SigLIP 隐藏层维度
        if hasattr(self.clip_text_encoder, 'config'):
            clip_dim = self.clip_text_encoder.config.hidden_size
        else:
            clip_dim = 1024
    ...
    def extract_feat(self, inputs, data_samples: SampleList, return_attns: bool = False) -> List[Tensor]:
        ...
        return sam_visual_feat, clip_text_feat_pooler, sam_logits

    def loss(self, inputs: Tensor, data_samples: SampleList) -> dict:
        """训练阶段入口"""
        # 提取原始特征
        # sam_high_res, local_feat, global_feat, sam_mask = self.extract_feat(inputs, data_samples)
        sam_high_res, global_feat, sam_mask = self.extract_feat(inputs, data_samples)
        losses = dict()
        ...
        feat_tuple_seg = (sam_high_res, global_feat, sam_mask_2ch)
        ...
        feat_tuple_cls = (sam_high_res, global_feat, sam_mask.detach())
    ...
    def predict(self, inputs: Tensor, data_samples: SampleList):
        ...
        sam_high_res, global_feat, sam_mask_logits = self.extract_feat(inputs, data_samples)
        # feat_tuple = (sam_high_res, local_feat, global_feat, sam_mask_logits)
        feat_tuple = (sam_high_res, global_feat, sam_mask_logits)
...
class SamSupervisionHead(BaseModule):
    ...
    def forward(self, inputs):
        ...
        if isinstance(inputs, tuple):
            ...
            # return inputs[3]
            return inputs[2] # 已经将clip_vision_feat删除，现在是(sam, text, mask)
        return inputs
...
class ReasoningClsHead(BaseModule):
    ...
    def __init__(self,
        ...
        # )
        self.fusion_conv   = nn.Sequential(
            nn.Conv2d(in_channels, in_channels, kernel_size=1), # 保留此网络，但是将维度大小修改一下
            nn.GroupNorm(32, in_channels),
            nn.GELU()
        )
    ...
    def forward(self, inputs):
        ...
        sam_v, poi_feat, sam_mask          = inputs
        ...
        fused_v    = self.fusion_conv(sam_v_proj) # 移除local_s，直接处理sam_v_proj
