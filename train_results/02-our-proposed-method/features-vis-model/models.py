...
# python tools/test.py --config configs/config-b.py --checkpoint work_dirs/config-b/best_epoch_5.pth
...
# python demo/inference_att_vis.py --config configs/config-b-inference.py --checkpoint H:/train_results/20260307/02-concat-fusion-0122/best_epoch_55.pth --out-dir inference_results-t-sne/02-concat-fusion-0122 --show-dir inference_results-t-sne/02-concat-fusion-0122
...
class RefSegEncoderDecoder(EncoderDecoder):
    ...
    def extract_feat(self, inputs, data_samples: SampleList, return_attns: bool = False) -> List[Tensor]:
        ...
        vis_dict = {} if return_attns else None # 可视化结果存放子字典
        ...
        sam_visual_out = self.backbone(x_sam)
        sam_visual_feat = sam_visual_out['last_hidden_state'] # 通过 SAM 主干网络提取高分辨率视觉特征
        # print("sam_visual_feat: ", sam_visual_feat.shape) # torch.Size([1, 256, 64, 64])
        print("sam vision self attention output：")
        print(type(sam_visual_out))
        print(sam_visual_out.keys())
        print(sam_visual_out['attentions'][-1].shape)
        # 保存 SAM attention
        if return_attns:
            vis_dict['sam_attention_last'] = sam_visual_out['attentions'][-1].detach().cpu()
        ####################################### SAM 视觉特征提取 (侧重高分辨率几何结构)
        ...
        clip_visual_out = self.clip_vision_encoder(x_clip)
        clip_visual_feat = clip_visual_out['last_hidden_state']
        ...
        print("clip vision self attention output：")
        print(type(clip_visual_out))
        print(clip_visual_out.keys())
        print(clip_visual_out['attentions'][-1].shape)
        # 保存 SigLIP vision attention
        if return_attns:
            vis_dict['siglip_vision_attention_last'] = clip_visual_out['attentions'][-1].detach().cpu()
        ####################################### SigLIP 视觉特征提取 (侧重深层语义感知)
        ...
        clip_text_out = self.clip_text_encoder(**text_dict)
        clip_text_feat_pooler = clip_text_out['pooler_output'] # 全局功能分类 / Scene token
        clip_text_feat = clip_text_out['last_hidden_state']    # BXSeqLenX1152 (局部 Token)
        # print(f"text_dict keys: {text_dict.keys()}")
        print("clip text self attention output：")
        print(type(clip_text_out))
        print(clip_text_out.keys())
        print(clip_text_out['attentions'][-1].shape)
        # 保存 text attention
        if return_attns:
            vis_dict['siglip_text_attention_last'] = clip_text_out['attentions'][-1].detach().cpu()
            vis_dict['poi_tokens'] = text_dict['input_ids'].detach().cpu()
        ####################################### POI 地理文本语义编码
        ...
        # 保存融合特征
        if return_attns:
            vis_dict['siglip_visual_feat'] = clip_visual_feat.detach().cpu()
            vis_dict['fusion_feat']        = local_fused_feat.detach().cpu()
        ############################# siglip视觉特征与POI语义特征的拼接融合
        ...
        # 保存 sam mask
        if return_attns:
            vis_dict['sam_mask_logits'] = sam_logits.detach().cpu()
        if return_attns:
            return sam_visual_feat, local_fused_feat, clip_text_feat_pooler, sam_logits, vis_dict
    ...
    def predict(self, inputs: Tensor, data_samples: SampleList):
        ...
        sam_high_res, local_feat, global_feat, sam_mask_logits, vis_dict = self.extract_feat(inputs, data_samples, return_attns=True)
...
class ReasoningClsHead(BaseModule):
    ...
    def forward(self, inputs):
        ...
        ############################## 1. 空间与维度对齐 # 阶段 1：SAM 几何 + SigLIP 语义 → 基础视觉融合特征
        ...
        ############################## 2. 构建精确证据池 # 阶段 2：视觉特征 + SAM 掩膜 → 掩膜加权的精准视觉证据
        # 直接使用 SAM 输出的掩膜进行插值
        ...
        final_evidence  = visual_evidence * (mask_weight + 0.1) # 掩膜加权融合：视觉特征 × 掩膜权重（+ 小偏置防止梯度消失）

        ############################## 3. 跨模态推理 # 阶段 3：功能原型 + POI 语义 → 动态自适应功能原型
        # 生成动态原型
        ...
        # 跨模态特征融合 # 阶段 4：动态原型 + 视觉证据 → 跨注意力推理融合
        # Reasoning: 用原型去匹配掩膜内的视觉证据
        updated_protos, attn_weights = self.reasoning_engine(
            prototypes,
            final_evidence,
            final_evidence,
            need_weights=True,
            average_attn_weights=False,
        ) # 核心推理：Query(原型) 在 Key/Value(视觉证据池) 中进行匹配

        ############################## 4. 全局上下文补偿 (残差连接) # 更新原型 + 全局视觉特征 → 残差补偿融合
        ...
        vis_reasoning = {
            'reasoning_attention' : attn_weights,
            'reasoning_prototypes': prototypes,
            'visual_evidence'     : final_evidence,
...
class RefSegSiglipTextModel(BaseModule):
    ...
    def forward(self, *args, **kwargs):
        kwargs['output_attentions'] = True # # 强制开启注意力输出
