...
class ReasoningClsHead(BaseModule):
    """
        功能：简单的特征拼接分类器 (Baseline for Ablation Study)。
        逻辑：Masked Pooling (视觉) + Concat (文本) -> MLP 分类。
        用于验证 ReasoningClsHead 中注意力机制和动态原型的有效性。
    """
    def __init__(self,
        ...
        # 简单的分类 MLP
        # 输入维度 = 视觉向量(in_channels) + 文本向量(in_channels)
        self.classifier = nn.Sequential(
            nn.Linear(in_channels * 2, in_channels),  # 融合层
            nn.LayerNorm(in_channels),
            nn.GELU(),
            nn.Dropout(dropout_ratio),
            nn.Linear(in_channels, num_classes)  # 输出层
        )
    ...
    def forward(self, inputs):
        ...
        # sam_v: [B, 256, 64, 64]
        # local_s: [B, 1024, H, W]
        # poi_feat: [B, 1024] (全局文本特征)
        # sam_mask: [B, 1, 256, 256] (Logits)
        ...
        ############################## B. 掩膜处理 (Masked Global Average Pooling)
        mask_sigmoid = torch.sigmoid(sam_mask)
        mask_resized = F.interpolate(mask_sigmoid, size=target_sz, mode='bilinear', align_corners=False)  # [B, 1, 64, 64]
        # 计算加权平均： (Feature * Mask).sum / Mask.sum
        # 加上 1e-6 防止除以零
        mask_sum     = mask_resized.sum(dim=[-2, -1]) + 1e-6                  # [B, 1]
        visual_vec   = (fused_v * mask_resized).sum(dim=[-2, -1]) / mask_sum  # [B, 1024]

        ############################## C. 特征拼接 (Concat)
        # visual_vec: [B, 1024] # poi_feat:   [B, 1024]
        combined_feat = torch.cat([visual_vec, poi_feat], dim=1)  # [B, 2048]
