...
from rsris.models.models import RefSegEncoderDecoder, RefSegSiglipVisionModel, RefSegSiglipTextModel, GATEncoder, RefSegSamVisionEncoder, RefSegSamPromptEncoder, RefSegSamMaskDecoder
...
resume   = False
# resume = True
# load_from = './work_dirs/PHASE2_CLS_ONLY/epoch_16.pth'
...
optim_wrapper = dict(
    ...
    paramwise_cfg=dict(
        custom_keys={
            ...
            # # # [核心投影]
            # # 'v_proj_head':         dict(lr_mult=1.0),
            # # 't_proj_head':         dict(lr_mult=1.0),
            # # 'text_to_vis_proj':    dict(lr_mult=1.0),
            # # # [关键参数]
            # # 'context_gate':        dict(lr_mult=1.0),
            # # 'poi_scale':           dict(lr_mult=1.0),

            # # ================= ReasoningClsHead =================
            # # [维度对齐]
            # 'sam_proj':            dict(lr_mult=1.0),
            # # [核心结构]
            # 'fusion_conv':         dict(lr_mult=1.0),
            # # 'proto_adapter':       dict(lr_mult=10.0),
            # # [推理引擎参数]
            # 'function_prototypes': dict(lr_mult=1.0),
            # 'reasoning_engine':    dict(lr_mult=1.0),
            # # [残差与决策]
            # 'feat_to_proto_proj':  dict(lr_mult=1.0),
            # 'consistency_head':    dict(lr_mult=1.0),
        }
    ),
