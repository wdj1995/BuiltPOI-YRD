...
class RefSegEncoderDecoder(EncoderDecoder):
    ...
    def set_finetune_parameters(self):
        ...
        peft_keys = ['backbone', 'clip_vision_encoder']
        for k in peft_keys:
            if hasattr(self, k):
    ...
    def extract_feat(self, inputs, data_samples: SampleList, return_attns: bool = False) -> List[Tensor]:
        ...
        # # Modality Dropout: 训练时随机丢弃 SigLIP 视觉特征，强迫模型依赖 POI
        # # 强迫对齐： 当视觉特征消失时，模型为了降低 Loss，必须学会利用 POI 文本。
        # # 提升鲁棒性： 防止模型过拟合图像纹理，真正理解“文本描述”与“地物”的关系。
        # if self.training:
        #     drop_visual_prob = 0.3
        #     if torch.rand(1, device=clip_visual_feat.device) < drop_visual_prob:
        #         clip_visual_feat = clip_visual_feat * 0.0
        ...
        return sam_visual_feat, clip_visual_feat, sam_logits

    def loss(self, inputs: Tensor, data_samples: SampleList) -> dict:
        """训练阶段入口"""
        # 提取原始特征
        # sam_high_res, local_feat, global_feat, sam_mask = self.extract_feat(inputs, data_samples)
        sam_high_res, clip_visual_feat, sam_mask = self.extract_feat(inputs, data_samples)
        losses = dict()
        ...
        feat_tuple_seg = (sam_high_res, clip_visual_feat, sam_mask_2ch)
        ...
        feat_tuple_cls = (sam_high_res, clip_visual_feat, sam_mask.detach())
    ...
    def predict(self, inputs: Tensor, data_samples: SampleList):
        ...
        sam_high_res, clip_visual_feat, sam_mask_logits = self.extract_feat(inputs, data_samples)
        # feat_tuple = (sam_high_res, local_feat, global_feat, sam_mask_logits)
        feat_tuple = (sam_high_res, clip_visual_feat, sam_mask_logits)
...
class SamSupervisionHead(BaseModule):
    ...
    def forward(self, inputs):
        ...
        if isinstance(inputs, tuple):
            ...
            # 解包元组，提取第3个元素 (sam_logits) # target_geo_feat, clip_visual_feat, sam_logits
            return inputs[2]
        return inputs
...
class ReasoningClsHead(BaseModule):
    ...
    def forward(self, inputs):
        ...
        sam_v, clip_visual_feat, sam_mask  = inputs
        ...
        clip_v     = F.interpolate(clip_visual_feat, size=target_sz, mode='bilinear', align_corners=False) # 统一缩放到 SAM 特征图尺寸 (64x64)
        # fused_v    = self.fusion_conv(torch.cat([sam_v_proj, local_s_up], dim=1))
        fused_v    = self.fusion_conv(torch.cat([sam_v_proj, clip_v], dim=1))
        ...
        prototypes        = self.function_prototypes.unsqueeze(0).expand(B, -1, -1)
